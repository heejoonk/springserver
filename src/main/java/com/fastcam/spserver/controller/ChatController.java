...
@RestController
@CrossOrigin({ "http://3.39.233.177:8000" , "http://3.39.233.177:3000", "http://3.39.233.177" })
public class ChatController {
    ...
    public ChatController() {
        this.restClient = RestClient.builder()
                ...
                .baseUrl("http://3.39.233.177:8000")
                .build();
    }

    @GetMapping("/test")
    public String index(){
        return "<h1>WELCOME DOCKER CICD WORLD!!!!!</H1>";
    }
    ...
    public ResponseEntity<byte[]> download(  @PathVariable String fileName) {
        ResponseEntity<byte[]> response = restClient.get()
                .uri("http://3.39.233.177:8000/download/" + fileName)
                .retrieve()
                .toEntity(byte[].class);
